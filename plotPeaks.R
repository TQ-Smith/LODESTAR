    ...
    if (length(args) != 1 && length(args) != 2) {
        ...
        if (!file.exists(fileName)) {
            ...
            if (useT) {
                ...
                manhat <- manhat[!manhat$P == 0,]
            }
            colnames(manhat) <- c("SNP", "CHR", "BP", "P")
            # Save plot to png.
            if (useT) {
                png(paste(substr(fileName, 1, nchar(fileName) - 4), "_t.png", sep = ""), width = 1600, height = 1200, res = 300)
                m <- manhattan(manhat, chr = "CHR", bp = "BP", p = "P", snp = "SNP", ylab = "-log(1-t)", main = substr(fileName, 1, length(fileName) - 4), xlim = c(min(manhat$BP - 1), max(manhat$BP + 1)), ylim = c(0, max(manhat$p) + 1))
            } else {
                png(paste(substr(fileName, 1, nchar(fileName) - 4), "_p.png", sep = ""), width = 1600, height = 1200, res = 300)
                m <- manhattan(manhat, chr = "CHR", bp = "BP", p = "P", snp = "SNP", main = substr(fileName, 1, length(fileName) - 4), xlim = c(min(manhat$BP - 1), max(manhat$BP + 1)), ylim = c(0, max(manhat$p) + 1))
            }
            dev.off()
        }
    }
